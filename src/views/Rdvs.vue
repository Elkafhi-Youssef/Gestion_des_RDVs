                    ...
                    >
                        ...
                        >
                            {{ el.val }}
                                ...
                                <td><button class="btn btn-success btn-sm rounded-0" @click="update();" >UPDATE <i class="fa fa-check"></i></button> &nbsp;<button class="btn btn-sm rounded-0" @click="cancel();"> <i class="fa fa-ban"></i></button></td>
                            </template>
                            <template v-else>
                                <td>{{ elemt.date }}</td> 
                                <td>{{ elemt.horaire }}</td>  
                                ...
                                <td>
                                    <button class="btn btn-danger btn-sm rounded-0" @click="slectU(elemt.id); del();" >DELETE
                                    <i class="bi bi-trash"></i></button> &nbsp;
                                    <button class="btn btn-primary btn-sm rounded-0" @click="edit(elemt.id);">UPDATE
                                    <i class="fa fa-pencil fa-fw" aria-hidden="true" style="color:#fff;"></i> </button></td>
                            </template>
                        </tr>
                    </tbody>
                </table>
...
export default {
  ...
  methods:{
    async getAll(){
    const response = await fetch('http://localhost/gestionRDVS/back-end/api/rdv/afficherRdv/'+this.ref);
    ...
    async del() {
        await fetch('http://localhost/gestionRDVS/back-end/api/rdv/supprimerRdv/'+this.curentElment,{
            method:"DELETE",
        });
       await this.getAll();
    },
    async edit(x){
        fetch('http://localhost/gestionRDVS/back-end/api/rdv/getoneRdv/'+x)
   ...
   async update(){
       ...
       await fetch(' http://localhost/gestionRDVS/back-end/api/rdv/modifierRdv/'+this.editClient.id,{
            method: 'PUT',
            headers:{
    ...
    cancel(){
     ...
     async getTime(val) {
            ...
            const response = await fetch(
                "http://localhost/gestionRDVS/back-end/api/rdv/afficherHr/" + val
            );
            const data = await response.json();
            this.horaires = data;
        },
